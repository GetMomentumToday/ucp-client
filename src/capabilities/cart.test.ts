...
describe('CartCapability.create', () => {
  it('sends POST /carts with payload and returns validated cart', async () => {
    mockOk(MINIMAL_CART);
    ...
    expect(url.endsWith('/carts')).toBe(true);
...
describe('CartCapability.get', () => {
  it('sends GET /carts/:id and returns validated cart', async () => {
    mockOk(MINIMAL_CART);
    const cart = await capability.get('cart_123');
    expect(cart.id).toBe('cart_123');
    const [url, init] = mockFetch.mock.calls[0] as [string, RequestInit];
    expect(url.endsWith('/carts/cart_123')).toBe(true);
  ...
  it('URL-encodes the cart ID', async () => {
    ...
    expect(url).toContain('/carts/cart%2Fspecial');
  });
...
describe('CartCapability.update', () => {
  it('sends PUT /carts/:id with payload and returns validated cart', async () => {
    mockOk(MINIMAL_CART);
  ...
  it('URL-encodes the cart ID on update', async () => {
    ...
    const [url] = mockFetch.mock.calls[0] as [string];
    expect(url).toContain('/carts/cart%2F123');
  });
...
describe('CartCapability.cancel', () => {
  it('sends POST /carts/:id/cancel and returns validated cart', async () => {
    mockOk(MINIMAL_CART);
    const cart = await capability.cancel('cart_123');
    expect(cart.id).toBe('cart_123');
    const [url, init] = mockFetch.mock.calls[0] as [string, RequestInit];
    expect(url.endsWith('/carts/cart_123/cancel')).toBe(true);
    expect(init.method).toBe('POST');
  });

  it('URL-encodes the cart ID on cancel', async () => {
    mockOk(MINIMAL_CART);
    await capability.cancel('cart/123');
    const [url] = mockFetch.mock.calls[0] as [string];
    expect(url).toContain('/carts/cart%2F123/cancel');
  });

  it('sends idempotency-key header on POST', async () => {
    mockOk(MINIMAL_CART);
    await capability.cancel('cart_123');
    const [, init] = mockFetch.mock.calls[0] as [string, RequestInit];
    const headers = init.headers as Record<string, string>;
    expect(headers['idempotency-key']).toBeDefined();
  });
  ...
  it('throws UCPIdempotencyConflictError on 409', async () => {
    mockError(409);
    await expect(capability.cancel('cart_123')).rejects.toBeInstanceOf(UCPIdempotencyConflictError);
  });

  it('throws UCPError on 404', async () => {
    mockError(404);
    await expect(capability.cancel('cart_missing')).rejects.toBeInstanceOf(UCPError);
  });

  it('throws UCPError on 500', async () => {
    mockError(500);
    await expect(capability.cancel('cart_123')).rejects.toBeInstanceOf(UCPError);
  });

  it('propagates network errors', async () => {
    mockNetworkFailure();
    await expect(capability.cancel('cart_123')).rejects.toThrow('Failed to fetch');
  });
});
