...
beforeEach(() => {
  ...
  capability = new CatalogCapability(http, { search: true, lookup: true });
});
...
describe('CatalogCapability.search', () => {
  ...
  it('falls back to raw data on schema validation failure', async () => {
    ...
    mockOk(malformed);
    const result = await capability.search('shoes');
    expect(result['unexpected']).toBe('shape');
  });
...
describe('CatalogCapability.getProduct', () => {
  it('sends POST /catalog/product with body and returns response', async () => {
    mockOk(MINIMAL_LOOKUP_RESPONSE);
    const result = await capability.getProduct('prod_123');
    expect(result).toEqual(MINIMAL_LOOKUP_RESPONSE);
    const [url, init] = mockFetch.mock.calls[0] as [string, RequestInit];
    expect(url.endsWith('/catalog/product')).toBe(true);
    expect(init.method).toBe('POST');
    const body = JSON.parse(init.body as string) as Record<string, unknown>;
  ...
  it('sends idempotency-key header on POST', async () => {
    mockOk(MINIMAL_LOOKUP_RESPONSE);
    await capability.getProduct('prod_123');
    const [, init] = mockFetch.mock.calls[0] as [string, RequestInit];
    const headers = init.headers as Record<string, string>;
    expect(headers['idempotency-key']).toBeDefined();
  });
  ...
  it('throws UCPIdempotencyConflictError on 409', async () => {
    mockError(409);
    await expect(capability.getProduct('prod_123')).rejects.toBeInstanceOf(
      UCPIdempotencyConflictError,
    );
  });

  it('falls back to raw data on schema validation failure', async () => {
    const malformed = { id: 'prod_123', unexpected: 'shape' };
    mockOk(malformed);
    const result = await capability.getProduct('prod_123');
  ...
  it('throws UCPError with gateway message on structured error', async () => {
    mockError(400, {
      messages: [{ type: 'error', code: 'PRODUCT_NOT_FOUND', content: 'Product not found' }],
    });
    const err = await capability.getProduct('prod_missing').catch((e: unknown) => e);
    expect(err).toBeInstanceOf(UCPError);
    expect((err as UCPError).code).toBe('PRODUCT_NOT_FOUND');
  });

  it('throws UCPError on 404', async () => {
    mockError(404);
    await expect(capability.getProduct('prod_missing')).rejects.toBeInstanceOf(UCPError);
  });

  it('throws UCPError on 500', async () => {
    mockError(500);
    await expect(capability.getProduct('prod_123')).rejects.toBeInstanceOf(UCPError);
  });

  it('propagates network errors', async () => {
    mockNetworkFailure();
    await expect(capability.getProduct('prod_123')).rejects.toThrow('Failed to fetch');
  });
});
